...
**Textures.** Tiles decode to raw RGBA and `Hio_StbImage` decodes encoded bytes only, so the boundary PNG-encodes on demand and caches per (tile, texture) — the cache is what makes the borrow valid for the frame's life. The URI (`tuile://tile/<id>/texture/<n>.png`) is built by `Frame::texture_uri` and by nothing else, because the material names it and the resolver is handed it back. The re-encode is real CPU per texture per frame; it is the cost of draping a private copy per tile and it goes away when imagery is referenced rather than baked.
